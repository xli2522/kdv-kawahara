...
def full_operation(pair):
    ...
    if damp_all_cases:
        optimized_u0, v0, stationary_u0 = analytical.optimize_u0Coeff(param_damping, a1, a1, 
                        steps=1, L=L, spaceResolution=N, plot=False, N=2*kModes+1, savePic=False, damping=damp_all_cases)
    ...
    #continue
    #visual.plot_video(sol, len(t), N, L, 'Table_test_'+str(int(L/np.pi))+'pi_'+str(mus)+'mu'+ '.avi', fps=30)
...
if __name__ == '__main__':
    ...
    current = time.time()
    pairs = [[5.09,62.82], [5.48,51.62], [4.79,35.98], [5.02,19.78], [4.16,7.09], [4.23,0.595], [3.24,-0.219], \
                    [-3.23,-0.305], [2.27,-3.22], [2.36,-13.41], [1.49,-29.71], [1.64,-49.13], [0.74,-64.87], [0.69, -57.60]]
    pool = Pool(processes=14)
    pool.map(full_operation, pairs)
    pool.terminate()
